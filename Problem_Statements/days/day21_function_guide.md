...
|----------|-----------:|----------------:|-----:|
| Simulate the VM to the first repeated probe | — | ~3.4 × 10⁹ | minutes (immutable-array VM) |
| Lift the hash, run it natively | ~38,600 | 0 | **2.3 ms** |

The simulated figure is ~88,000 VM instructions per probe, of which
all but ~30 are the trial-divide loop — the device spends **~99.9%
of its time dividing by 256**, because its ISA has no shift. The
...
- **An ISA tells you where the time goes.** No shift instruction ⇒
  dividing by 256 costs thousands of trial multiplications ⇒ 99.9%
